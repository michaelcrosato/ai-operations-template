...
### Added
- **External-tool registry activated — registry-first, zero tools enabled (2026-07-17).** The optional module cataloged since June ships ahead of first live use, per the standing rule that any external CLI/MCP tool needs a registry entry *before* integration: `.claude/tool-policy.json` (machine-readable — purpose, trust level, env-named secrets, allowed commands, network needs, approval gate per tool) plus `docs/tool-registry.md` (the integration doctrine: an external CLI runs only as a sandboxed one-shot leaf worker whose output is captured to evidence and re-validated by this repo's own gate and fresh-context evaluator — never a peer orchestrator). Three candidate lanes registered with adversarially-verified confidence notes (OpenAI Codex CLI — most verified; xAI Grok Build — subscription tier unresolved; Google Antigravity — headless capability unresolved); every entry gates on operator approval, and the enable/fund decision is escalated to the operator, not made. Claude Code sub-agent model policy is untouched; no hooks were added or changed.
...
### Changed
- **README principles rewritten 6 → 7, plus a whole-README truth-up (2026-07-17).** The public principles now lead with "Trust nothing without evidence" (previously absent from the README set despite being the engine's most-enforced rule), promote risk-tiered autonomy and "every line must earn its keep" (the repo's stated bet that a harness's durable core is verification + state + guardrails; everything else stays small and deletable) to principles, merge the two orchestrator-conduct principles into one, and drop operator-as-business-client from the README (it remains P3 in the ops plan). Grounded in a four-stream live research pass (instruction-file conventions, guardrail/evidence norms, template-repo landscape, simplicity-vs-scaffolding) whose durable findings are written back to `docs/FRONTIER.md` §4. Facts trued: ~4,600 lines of scripts/hooks (~6,000 installed), 464 hook-contract tests re-verified green, status stamps refreshed, an internal 350+/464 contradiction unified, and a new honest limitation naming the engine's own footprint growth as the number to watch. An adversarial 4-lens review pass then tightened the rewrite's own evidentiary claims before the PR (consensus→attributed bet, venue-year/model-vintage fix on the reward-hacking citation, like-for-like footprint trend).
...
### Docs
- **`docs/repo-explorer.html` — an interactive, first-principles tour of every code file (2026-07-18).**
  A self-contained single-page dashboard (light + dark) that explains all 65 non-doc files from zero:
  a 7-layer mental model, an animated 8-station loop, a searchable 10-cluster explorer with a
  progressive-disclosure drawer per file (one-sentence version → mechanism → gotchas → cross-links),
  and a glossary. Built as a "modern AI-coded repo 101"; explanations generated by reading the actual
  files, with real line counts and load-bearing claims verified against source. Linked from the README.
