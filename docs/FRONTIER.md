...
| 2026-07-17 | External coding-agent CLIs all real (adversarial pass: 92 claims, 25 vote-tested → 18 confirmed / 7 killed): **Codex CLI** (openai/codex, Apache-2.0, ~99k stars; headless `codex exec`; CONFIRMED sandbox modes read-only/workspace-write/danger-full-access with OS-native enforcement — Seatbelt/bubblewrap/native Windows — network **off by default**); **Grok Build** (xai-org/grok-build, open-sourced ~2026-07-14; TUI + headless `grok -p … --output-format streaming-json` + ACP; `grok inspect` documents "config sources, instructions, skills, plugins, hooks, and MCP servers"; **no vendor sandbox found in primary docs** — runs locally); **Antigravity CLI** (launched ~2026-05-19; **Gemini CLI retired in its favor**, subscription serving stopped 2026-06-18 — Antigravity is the forward-looking Google target). Registry: `.claude/tool-policy.json` + `docs/tool-registry.md` | docs.x.ai/build · developers.openai.com/codex · Google dev blog + trade press · DECISIONS 2026-07-17 |
| 2026-07-17 | External-CLI auth is subscription-capable on all three (subscription-first compatible): Codex `codex login` ChatGPT OAuth **co-equal** with `--with-api-key` (the "OAuth-first" hierarchy claim was REFUTED 0-3), device-code beta for headless; Grok Build browser OAuth default + `XAI_API_KEY` documented headless fallback — **required subscription tier CONFLICTING** across sources ($30–$300/mo claims; primary docs name no tier); Antigravity Google Sign-In + OS keyring (2-source corroborated, UNTESTED), $100/mo AI Ultra for higher tier (UNTESTED) | developers.openai.com/codex/auth · docs.x.ai/build/cli/headless-scripting |
| 2026-07-17 | External-CLI claims KILLED or open — do not re-cite as fact. KILLED: Codex Ultra-mode subagents as a builder/evaluator analog (OpenAI's own docs: *"there is no separate verification or judge role distinct from workers"* — homogeneous workers, no judge; route external output through THIS repo's evaluator); Antigravity "working multi-agent orchestration for real coding tasks" and "closest analog to this repo" (refuted; independent reviews found orchestration unreliable). OPEN/UNVERIFIED: **Antigravity headless/CI mode** (single source says it LACKS one — hard blocker for this repo's non-interactive loop if true; verify FIRST); Grok Build "8 concurrent worktree subagents"/"Arena Mode" (non-primary only); Codex MCP client+server both-direction (2 blogs, no primary check) | 2026-07-17 research synthesis (19 sources) · DECISIONS 2026-07-17 |
