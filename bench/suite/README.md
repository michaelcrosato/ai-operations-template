...
| `H2-shared-docs` | H2 | headroom (engine-effect) | import-the-module, 6 fns / 8 rules: create/validate, owner CRUD, share (view/edit) (primary) + **GATING access cruxes** — view≠edit, owner-only delete, owner-only share, `listDocs` no-leak | **validity-gated** (ref 1.0; each dropped crux → gated 0). **Also ceiling:** bare sonnet one-shot **4/4 = 1.0** — strong models handle multi-rule access control one-shot, so no engine-effect headroom on greenfield (see [`../ENGINE-EFFECT-PLAN.md`](../ENGINE-EFFECT-PLAN.md) §9; the signal route is refactoring/regression, not bigger greenfield). |
