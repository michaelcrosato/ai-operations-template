...
**Phase-1 status: DONE.** The first task — `bench/suite/L3-mcp-calc-search/` ("build an MCP server") — is built and **proven end-to-end**: its **validity gate is green** (the oracle scores the reference 1.0 and catches off-by-one, hardcoded/overfit, missing-tool, crash, and corpus-tamper), and a **live Sonnet build scored 1.0** (held-out 4/4) in **3 turns / $0.13 / 31s**. The machinery — agent builds → oracle verifies (primary + held-out + anti-cheat) → blueprint telemetry — works. **The L4 CRM flagship is also built** (`bench/suite/L4-crm-api/`): an HTTP acceptance oracle with **gating criteria** (RBAC + referential integrity are pass-required — a broken-RBAC build scores 0, proven by its validity gate, not a deceptive partial); a live Sonnet build scored **0.93** (gates passed, one primary miss). Next: the L1 rung, then the L5 hard tier — the L4-torture/circuit-breaker stress task, the security-by-abuse-tests task, and the ambiguity trap.

**Gauntlet status (2026-06-16): BUILT — the full harness-property "3DMark" (G1–G4) is live, validity-gated, and dogfooded `pass^2`.** The operator's `suggestedtests.md` (four independent assistants converging on the same four layers) is folded in as the **G-tier** (the engine-effect axis), distinct from the capability-ladder L-tasks: **`G1-strict-json`** (output discipline — raw-bytes parse/no-fence/minified/exact-keys over a trap-laden CSV transform), **`G2-context-trace`** (long-context cross-cutting trace with a *gating* peripheral-integrity group + a live behavioral probe through the real module graph), **`G3-dag-pipeline`** (mandated Read→Plan→Write→Validate, 7 held-out cases as anti-cheat), **`G4-circuit-breaker`** (unresolvable-env `BLOCKED.md` halt + anti-cheat DQ). Each scored `pass^2` (1/1/1) with every group at full marks; `run-suite.mjs` gained backward-compatible nested-fixture support for G2's monorepo. The design was **live-verified** against real 2026 eval literature in `bench/HARNESS-RESEARCH.md` (Harness-Bench arXiv 2605.27922, "Stop Comparing LLM Agents Without Disclosing the Harness" 2605.23950 — a 9.5-pt harness swing, ImpossibleBench's 54%→9% abort-affordance result, EvilGenie, tau-bench `pass^k`), which also catalogs the fabricated/misattributed citations in the desktop notes to keep out of the repo (§3 there). Next: deepen the loop/circuit-breaker telemetry (step-level fuse + semantic loop detection, HARNESS-RESEARCH §4.4) and run the engine-effect comparison — now scoped in `ENGINE-EFFECT-PLAN.md` (model-pinned arms A0 baseline → A4 full loop; the experimental harness + the A1 context arm built 2026-06-16; the §6.5 caveat governs — categorical signal on a headroom task, not a continuous lift on today's saturated tasks).
