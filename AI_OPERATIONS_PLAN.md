# AI Operations Plan — <REPOSITORY_NAME>
...
- **Nothing reaches protected `main` without passing machine gates** (type checks, tests, security tests, a fresh-eyes AI evaluator), **and nothing becomes a tagged release without your click-through QA.** Agents merge short-lived PR branches into `main` when all machine gates are green; release tags and GitHub Releases mark the milestones you accepted.
...
| Code host & gate | **GitHub** (`<GITHUB_ORG_OR_USER>/<REPO_NAME>`) with branch protection + **claude-code-action v1 (GA)** for `@claude` mentions and CI autofix | PRs are the unit of review; CI is the objective referee; `@claude` lets the operator turn a plain-English comment into a fix without opening anything else. |
| App hosting / QA surface | **<QA_DEPLOYMENT_SURFACE>** (e.g. Vercel, Netlify, Fly.io, Cloudflare) — automatic preview deployment per PR + persistent staging deployment from `main` | A non-technical human QAs by clicking a URL, never by running anything. |
| Database | **<DATABASE_SERVICE>** (e.g. Supabase, CockroachDB, RDS) — one production project + one development/staging project; migrations applied by agents via CLI in CI | Keeps destructive experimentation off production. Ephemeral database preview branches (or schema-isolated test databases) isolate e2e runs from the shared dev project (§7.1). |
| E2E verification | **<E2E_TEST_FRAMEWORK>** (e.g. Playwright, Cypress) driven by agents ("test like a human user") against seeded data | Anthropic's harness research found agents skip end-to-end verification unless given browser tooling and told to use it; this closed the biggest quality gap in their long-running trials. |
...
| Tier | Used by | Current mapping (verified 2026-07-16) | Notes |
|---|---|---|---|
| `reasoning` | Orchestrator, planner (`/groom`), evaluator, security-reviewer, **builder-strong** (Tier C builds) | `opus` → Opus 4.8 (`claude-opus-4-8`), $5/$25 per MTok, `high`/`xhigh` effort | Long-horizon autonomous execution; review quality is the last line of defense — never economize here. **Fable 5** (`claude-fable-5`, GA 2026-06-09) at $10/$50 per MTok — 2× Opus 4.8 on both input and output — is a valid mapping for the hardest planning/evaluation passes; the cost line in STATUS.md tracks its billing (§8.2). *Carried forward UNVERIFIED (outside the 2026-07-16 fetched scope, and its stated date has now passed — re-verify before relying on it): the "included on subscription plans through Jun 22, 2026, then usage credits" terms.* |
| `builder` | Builder sub-agent (Tier A/B routine implementation) | `sonnet` → **Sonnet 5** (`claude-sonnet-5`, released 2026-06-30), `high` effort | Best speed/intelligence balance for well-specified single features. Pricing is two-phase: **$2/$10 per MTok introductory through 2026-08-31 (40% of Opus 4.8), then $3/$15 standard from 2026-09-01 (60%)**. Sonnet 5 superseded Sonnet 4.6 here three weeks after the previous stamp — the alias moved, so this row named the wrong generation until 2026-07-16. |
| `fast` | Explorer / search / triage sub-agents | `haiku` → Haiku 4.5 (`claude-haiku-4-5`), $1/$5 per MTok, `low`/`medium` effort | Cheap fan-out for "find where X lives" work; returns conclusions, not file dumps. |

> **The aliases are provider-dependent.** Claude Code resolves a bare alias to a built-in default that differs per infrastructure backend, and the docs tabulate that variance for **`opus` and `sonnet` only** (verified 2026-07-16, [model-config](https://code.claude.com/docs/en/model-config)): `sonnet` → **Sonnet 5** on the Anthropic API, Sonnet 4.6 on Claude Platform on AWS, Sonnet 4.5 on Bedrock / Google Cloud's Agent Platform; `opus` → Opus 4.8 on all of those, but Opus 4.6 on Microsoft Foundry. The same page says aliases "such as `fable`, `opus`, `sonnet`, and `haiku`" resolve to a per-provider built-in default, but publishes **no per-provider table for `haiku`** — so haiku is provider-dependent *in principle* and unverified *in specifics*; do not assume it is stable across backends. This engine is Anthropic first-party (subscription login, §7.2), which is what the table above describes — `model-policy.json`'s `_provider` field records that dependency so a future backend change re-verifies instead of silently changing tiers.

> **Per-token prices are only comparable within a tokenizer generation** (verified 2026-07-16, [pricing](https://platform.claude.com/docs/en/about-claude/pricing) + [token-counting](https://platform.claude.com/docs/en/build-with-claude/token-counting)). Opus 4.7-and-later (so Opus 4.8), Fable 5, and Sonnet 5 use a **newer tokenizer producing ~30% more tokens for the same text**; Sonnet 4.6 and earlier — and Haiku 4.5 — use the previous one. Consequences: the `builder`-vs-`reasoning` ratios above (40%/60%) are apples-to-apples because Sonnet 5 and Opus 4.8 share the new tokenizer; but **`fast` (Haiku 4.5) is on the old tokenizer, so its $1/$5 is *not* directly comparable per-token to the other two tiers** — the same text bills fewer tokens there. Likewise "Sonnet 5 vs Sonnet 4.6" is not a pure price comparison. *Magnitude: ~30% typical with a ~35% ceiling — pricing/token-counting state "approximately 30%" (a central estimate, no ceiling given); the Opus 4.8 migration guide gives the range "roughly 1x to 1.35x ... (up to ~35% more, varying by content)". Consistent, not contradictory — but treat ~30% as a planning estimate, not a constant.*
...
5. Git rules: branch naming, never push directly to protected `main`, PR template requirements.
...
│ 6. SHIP    orchestrator opens PR feat/F-XXXX → main (template §8.3) —      │
│            PR creation is this session's job, never assumed from CI bots.  │
│            When CI is green: merge to main, flip passes:true via           │
...
- **Allow:** `<PACKAGE_MANAGER>` scripts, `<RUNTIME>`, `<COMPILER>`, `<LINTER>`, `<TEST_RUNNER>`, `<DB_CLI>` CLI (dev instances only), `git` (add/commit/push/branch/fetch/pull), `gh pr` equivalents via the GitHub integration, file edits repo-wide.
- **Deny (hard):** direct `git push` to protected `main` (and legacy `master`), force-push, `rm -rf` outside workspace temp, reading `.env*`/secret stores, `curl|sh`-style pipe-to-shell, package publishing, or modifying production database configurations.
...
| `Pre-Commit` (Git Hook) | `assertion-shield.ts` | Run `npx ts-node scripts/assertion-shield.ts` to block commits that delete or weaken test assertions compared to `origin/main` |
...
- **Branch invariant (hard rule):** `main` is the repository's **default and only long-lived branch** on GitHub (§11) — cloud sessions and Routines clone the default branch, so every tool lands on the canonical base automatically. Every autonomous run begins with `git fetch origin main` and branches from `origin/main`; every PR targets `main`, and its short-lived source branch is deleted after merge. Work discovered to be based on the wrong branch is redone from `main`, not force-rebased.
- **Revert-first (code), roll-forward (database):** if `main` CI breaks, the fixing session's first move is a revert PR for the offending merge (restore green), then re-attempt on a new branch. **Exception:** merges containing applied database migrations are never git-reverted — reverting deletes migration files while the staging schema stays mutated, desynchronizing ORM/client state from the database. Schema problems roll *forward* with a new corrective migration. Staging never stays red overnight either way.
- **Branch protection (GitHub settings):** `main` blocks direct/force pushes and deletion, requires a PR plus the green `verify` check, and applies those rules to admins. Tier A/B PRs may merge automatically after the machine gates; Tier C / REQUIRE_APPROVAL PRs remain held by the state-machine approval gate. All agent branches are short-lived `feat/F-XXXX`, `fix/...`, or `chore/...` branches; cloud sessions may use `claude/`-prefixed branches for ad-hoc fixes.
- **Record-PR pattern (state under branch protection):** because protected `main` accepts no direct writes, feature state (`features.json` entry, `--status`/`--passes` flips, evidence) rides the feature PR itself (per the state-flip convention, DECISIONS 2026-06-09), and post-merge records (PROGRESS/STATUS/metrics/kaizen notes) land via short-lived `chore/` branch PRs merged on green CI. This is the designed shape under branch protection, not a workaround.
- **Disaster floor:** worst case is always "revert to the last green commit on `main`" — which the human can do with one click, documented in OPERATOR_GUIDE.md.
...
At phase milestones (or weekly), the orchestrator prepares `/qa-pack` for everything merged since the last release tag: a numbered, click-by-click script in plain English ("1. Open staging.../dashboard. 2. Enter value X. 3. You should see results Y..."), with screenshots of expected results. The operator follows it on the staging URL and comments on anything off (`@claude step 3 showed error Z`). Once it passes, the orchestrator creates an annotated tag and GitHub Release at the tested `main` commit. Publishing that release — not creating another long-lived branch — is the release event. For security-sensitive releases, the operator may additionally paste the qa-pack/diff into a *different* frontier model for a heterogeneous second opinion to avoid same-family LLM review blind spots.
...
**Morning (5–10 min):** read `roadmap/STATUS.md` (or the latest GitHub Release notes). Edit `ROADMAP.md` on github.com directly — reorder bullets, add ideas in plain English, answer anything new in `QUESTIONS.md` inline. Open claude.ai/code → "Continue the roadmap" (until nightly Routines take over, then this step disappears).
...
Each phase ends with a human QA gate and an annotated release tag on `main`. The product scope inside each phase comes from the core project roadmap; `/groom` turns them into `features.json` entries at phase start.

| Phase | Scope | Exit criteria (human QA gate) |
|---|---|---|
| **0 — Build the factory** | Everything in §3–§8 of this plan: CLAUDE.md, roadmap/ files seeded (~20–50 features from requirements via `/groom`), features.schema.json + update-state.ts, model-policy.json, agents, skills, rules, hooks with contract tests, settings, security-guidance plugin configuration, scripts, CI workflows, branch protection + `main` as the only long-lived/default branch, PR/STATUS templates, OPERATOR_GUIDE.md | Operator can read STATUS.md, ROADMAP.md, OPERATOR_GUIDE.md and confirm they make sense; a deliberately-trivial feature (e.g. the `src/health.js` engine health-check CLI) flows the entire loop: brief → build → verify → evaluate → PR → CI → main, with correct PROGRESS/evidence artifacts |
...
2. GitHub: set `main` as the repository's **default and only long-lived branch**, then protect it with required PRs + the green `verify` check, rules enforced for admins, and force-pushes/deletions blocked. Release milestones use tags and GitHub Releases (§7.4), not a second branch. ☐
