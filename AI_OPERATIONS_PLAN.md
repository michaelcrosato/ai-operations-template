...
|---|---|---|---|
| `reasoning` | Orchestrator, planner (`/groom`), evaluator, security-reviewer, **builder-strong** (Tier C builds) | `opus` → Opus 4.8 (`claude-opus-4-8`), $5/$25 per MTok, `high`/`xhigh` effort | Long-horizon autonomous execution; review quality is the last line of defense — never economize here. **Fable 5** (`claude-fable-5`, GA 2026-06-09) at $10/$50 per MTok — 2× Opus 4.8 — is a valid mapping for the hardest planning/evaluation passes; the cost line in STATUS.md flags the subscription→usage-credit transition (§8.2). *Unverified in the 2026-07-16 pass and carried forward: the "included on subscription plans through Jun 22, 2026" terms.* |
...
> **The aliases are provider-dependent.** `opus`/`sonnet`/`haiku` resolve differently per infrastructure backend (verified 2026-07-16, [model-config](https://code.claude.com/docs/en/model-config)): `sonnet` is Sonnet 5 on the Anthropic API but Sonnet 4.6 on Claude Platform on AWS and Sonnet 4.5 on Bedrock/Google Cloud; `opus` is Opus 4.6 on Microsoft Foundry. This engine is Anthropic first-party (subscription login, §7.2), which is what the table above describes — `model-policy.json`'s `_provider` field records that dependency so a future backend change re-verifies instead of silently changing tiers.
