...
| App hosting / QA surface | **<QA_DEPLOYMENT_SURFACE>** (e.g. Vercel, Netlify, Fly.io, Cloudflare) — automatic preview deployment per PR + persistent staging deployment from `develop` | A non-technical human QAs by clicking a URL, never by running anything. |
...
| `PostToolUse` (Edit/Write) | format hook | Format/Lint-fix the touched file (keeps diffs clean, prevents lint-debt loops) |
...
Hook mechanics follow the current hooks reference: simple denials use exit code 2; richer control (allow/deny/ask + feedback to the model) uses the JSON `hookSpecificOutput.permissionDecision` output. `commit-on-stop.sh` registers on both `Stop` and `StopFailure`. A PreToolUse edit guard additionally enforces the active feature's `authorized_paths`/`forbidden_paths` from `features.json` (§4.2), so scope discipline is mechanical rather than prompt-obedience. Phase 0 ships **contract tests with fixtures for every hook script** — the safety net is tested code, not vibes.
...
- **claude.yml (claude-code-action, pinned to an exact commit SHA at or beyond v1.0.94, rolled forward by Dependabot):** responds to `@claude` comments and, on CI failures on agent PRs, triages and pushes fix commits to the PR branch (capped `--max-turns`, concurrency-limited). It pushes commits; opening PRs remains the orchestrator session's job — no workflow assumes the action creates PRs. The version floor is load-bearing: releases before v1.0.94 contain a publicly disclosed permission-check flaw (any `[bot]` actor trusted unconditionally) exploitable via prompt injection for secret exfiltration and repository compromise; fixed versions enforce human-actor checks and block bot triggers by default — our workflows never re-enable them. Billing note: from **Jun 15, 2026**, GitHub Actions agent runs bill against the separate per-user agent credit pool, not subscription usage (§5.2).
...
2. GitHub: set `develop` as the repository's **default branch**, then confirm branch protection on the stable branch (PR + 1 approval) and `develop` (PR + green CI). ☐
