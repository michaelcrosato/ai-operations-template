...
2. **Ready for your QA** — features `done` since the most recent release tag: staging/preview link + one sentence each. Empty → say "Nothing yet."
...
Data sources: `features.json` (truth for status), `roadmap/metrics.jsonl` (pass rates, review outcomes, cost notes — feed the Health line from data, not prose), `git log origin/main` since last report, open PRs (`gh pr list`), CI runs (`gh run list`), QUESTIONS.md. Facts only — if staging is broken, say so plainly; never soften health to look good.
