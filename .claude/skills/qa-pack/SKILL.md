---
name: qa-pack
description: Produce the click-by-click human QA script for everything newly done since the last release tag. Attach it to the release checklist before a GitHub Release is published. The only artifact the operator uses to accept a release.
---

# /qa-pack — the human QA script

For every feature `done` since the last release tag, write a numbered click-through script a non-technical person follows on the staging URL. 8th-grade reading level.
...
- End the pack with: total time estimate, the one-click rollback reminder (OPERATOR_GUIDE §4), and "publishing this release = releasing to users."
